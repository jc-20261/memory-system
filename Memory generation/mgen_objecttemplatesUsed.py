#!/usr/bin/env python3
r"""
mgen_objecttemplatesUsed.py

This is the latest mgen.py plus one focused change:

At the decompose/enhance stage, the pipeline now passes the current object
template storage as a separate system prompt. The LLM is asked not only to
decompose the action plot and object list, but also to match each object in the
object list against existing object templates where possible.

This file is intentionally separate from mgen.py. Future updates to mgen.py
will not automatically update this file unless explicitly requested.
"""
...
# Configuration
# =============================================================================
DATA_DIR = r"F:\New folder (4)\New folder"
...
class TemplateMatcher:
    ...
    def find_matching_object(self, proposed_name):
        if not self._obj_bm25 or not self._obj_emb:
            return None
        ...
        if proposed_sub_actions:
            ...
            for name in self._act_names:
                ...
                if stored_seq and stored_seq == seq:
                    ...
                    return name
        if not self._act_bm25 or not self._act_emb:
            return None
...
class MemoryGenerationPipeline:
    def __init__(self, llm, object_storage, action_storage, search_engine,
        ...
        # NEW: Build object-storage prompt, used at the decompose/enhance stage.
        self.object_storage_prompt = self._build_object_storage_prompt()
    ...
    def _build_object_storage_prompt(self) -> str:
        """
        Build a concise object-template storage prompt for the decompose stage.

        Includes template names, aliases, categories, functions, and materials.
        Does not include every default attribute to avoid extremely large prompts.
        """
        lines = [
            "OBJECT TEMPLATE STORAGE",
            "The following object templates already exist in the memory system.",
            "When extending/decomposing the object list, match each object against these templates where possible.",
            "If an object matches an existing template, use that template name.",
            "If a closely related object is found, extend the existing template instead of creating a duplicate.",
            "",
        ]

        for name in sorted(self.object_storage.list_templates()):
            aliases = self.object_storage.get_aliases(name)
            cats = self.object_storage.get_categories(name)
            funcs = self.object_storage.get_functions(name)
            mats = self.object_storage.get_materials(name)
    ...
    async def _encode_memory_advanced(self, nl_text, activity, timings: dict):
        ...
        # Stage 2: Enhanced sketch / decompose WITH object storage prompt
        t0 = time.perf_counter()
        combined_enhance_msg = self.enhance_system_msg + "\n\n" + self.object_storage_prompt
        enhanced, enhanced_usage = await self._call_llm_high("enhance_sketch", combined_enhance_msg, sketch, activity)
    ...
    # ... rest of methods identical to latest mgen.py, but included in file for completeness
    # (validate_review_output, output_tokens, parse_json, call_llm_high, normalize, find_template, resolve_goal_object, ingest, etc.)
